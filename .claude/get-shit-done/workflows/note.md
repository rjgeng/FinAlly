...
- **Global scope**: `/Users/rfvitis/temp/FinAlly/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/rfvitis/temp/FinAlly/.claude/notes/`)
...
2. Glob `/Users/rfvitis/temp/FinAlly/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/rfvitis/temp/FinAlly/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/rfvitis/temp/FinAlly/.claude/notes/` — works in any directory
